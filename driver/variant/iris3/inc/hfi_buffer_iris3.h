...
#define HFI_IRIS3_ENC_RECON_BUF_COUNT(num_recon, n_bframe, ltr_count, \
	_total_hp_layers, _total_hb_layers, hybrid_hp, codec_standard, is_multiview) \
	do \
	{ \
		HFI_U32 num_ref = 1; \
		if (n_bframe) \
		...
		{ \
			...
			{ \
				...
				if (is_multiview && _total_hb_layers == 2) \
					num_ref = num_ref * 2;                 \
			} \
			else if (codec_standard == HFI_CODEC_ENCODE_AVC) \
			{ \
...
#define HFI_IRIS3_ENC_MIN_INPUT_BUF_COUNT(numInput, TotalHBLayers, isMultiView) \
	do                                                                \
	{                                                                 \
		numInput = 3;                                             \
		if (TotalHBLayers >= 2)                                   \
		{                                                         \
			...
			if (isMultiView && TotalHBLayers == 2)            \
				numInput = 5;        \
		}                                                         \
	} while (0)
