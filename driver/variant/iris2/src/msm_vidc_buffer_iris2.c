...
static u32 msm_vidc_decoder_dpb_size_iris2(struct msm_vidc_inst *inst)
{
	...
	if (color_fmt == MSM_VIDC_FMT_NV12) {
		...
		HFI_NV12_UBWC_IL_CALC_BUF_SIZE_V2(size, width, height,
			video_y_stride_bytes(color_fmt, width), video_y_scanlines(color_fmt, height),
			video_uv_stride_bytes(color_fmt, width), video_uv_scanlines(color_fmt, height),
...
static u32 msm_vidc_encoder_comv_size_iris2(struct msm_vidc_inst *inst)
{
	...
	if (inst->codec == MSM_VIDC_H264)
		HFI_BUFFER_COMV_H264E(size, width, height, num_recon, profile, lookahead_enable);
	else if (inst->codec == MSM_VIDC_HEVC || inst->codec == MSM_VIDC_HEIC)
		HFI_BUFFER_COMV_H265E(size, width, height, num_recon, profile, lookahead_enable);
...
static u32 msm_vidc_encoder_non_comv_size_iris2(struct msm_vidc_inst *inst)
{
	...
	if (inst->codec == MSM_VIDC_H264)
		HFI_BUFFER_NON_COMV_H264E(size, width, height, num_vpp_pipes, profile, lookahead_enable);
	else if (inst->codec == MSM_VIDC_HEVC || inst->codec == MSM_VIDC_HEIC)
		HFI_BUFFER_NON_COMV_H265E(size, width, height, num_vpp_pipes, profile, lookahead_enable);
...
static int msm_buffer_dpb_count(struct msm_vidc_inst *inst)
{
	...
	if (is_decode_session(inst)) {
		color_fmt = inst->capabilities[PIX_FMTS].value;
		if (is_linear_colorformat(color_fmt))
			count = inst->fw_min_count ? inst->fw_min_count : inst->buffers.output.min_count;
		return count;
	}
