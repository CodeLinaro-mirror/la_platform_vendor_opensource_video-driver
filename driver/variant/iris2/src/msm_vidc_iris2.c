...
static int __power_off_iris2_hardware(struct msm_vidc_core *core)
{
	...
	if (is_core_sub_state(core, CORE_SUBSTATE_FW_PWR_CTRL) && is_iris2_hw_power_collapsed(core)) {
...
static int __boot_firmware_iris2(struct msm_vidc_core *vidc_core)
{
	...
	while (!ctrl_status && count < max_tries) {
		...
		if (ctrl_status & 0x1) {
			d_vpr_e("%s: boot firmware is successful, ctrl status %#x\n",
...
static struct msm_vidc_session_ops msm_session_ops = {
	...
	.scale_clocks = msm_vidc_scale_clocks_iris2,
